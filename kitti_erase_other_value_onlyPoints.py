...
kitti_label_tosave_path = 'kitti_erase_other_values/val_label/'
if not os.path.exists(kitti_label_tosave_path):
    os.makedirs(kitti_label_tosave_path)
...
# names.names -> class label
kitti_names = open('example/kitti/names.names','r')
...
f = open('kitti_erase_other_values/train.txt','w')
...
for indexi in range(len(kitti_images)):
    ...
    label_name = os.path.basename(kitti_img_totest_path).split(".")[0]+".txt"
    kitti_label_totest_path = kitti_label_path +  label_name
    ...
    real_label = open(kitti_label_tosave_path + label_name,'w')
    ...
    for line in label_contents:
        ...
        if(len(data) == 15):
            ...
            if(class_str != 'DontCare'):
                ...
                line_to_write = str(kitti_names_num[class_str]) + ' ' + str(intx1)+ ' ' + str(inty1)+ ' ' + str(intx2)+ ' ' + str(inty2) +'\n'
